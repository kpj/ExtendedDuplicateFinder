...
def gen_keylist(opts):
	...
	return [k for k in Item().keys() if (bool(opts["negate_all_options"]) ^ bool(opts[k])) or opts["compare_all"]]
...
def gen_parser(cmd):
	...
	cmd.parser.add_option(
		...
		action = 'store_true',
		help = 'Negate all passed options, e.g. "--title --negate" would compare everything but the title (has no effect with --all)'
	)
	cmd.parser.add_option(
		'--all',
		dest = 'compare_all',
		action = 'store_true',
		help = 'Compares all available options, i.e. only finds exact matches (takes precedence over --negate)'
	)
